...
function wgcal_storeevent(&$action) {
  ...
  $event->setValue("CALEV_EVTITLE", GetHttpVars("rvtitle", _("no title")));
  ...
  $htype = 0;
  if (GetHttpVars("evtimemode", 0) == 1) {
    $htype = 1;
    $start =date("d/m/Y 00:00:00",$ds);
    $end = date("d/m/Y 00:00:00",$ds);
  }
  if (GetHttpVars("evtimemode", 0) == 2) {
    $htype = 2;
    $start =date("d/m/Y 00:00:00",$ds);
  ...
  $rweekday = GetHttpVars("evrweekday", array());
  print_r2($rweekday);
  $daymask = 0;
  if ($rmode==2) {
  ...
  $date = GetHttpVars("evruntildate", 0);
