 ...
 *
 * @author Anakeen 2000 
 * @version $Id: search_fulltext.php,v 1.6 2003/08/18 15:47:03 eric Exp $
...
// ---------------------------------------------------------------
// $Id: search_fulltext.php,v 1.6 2003/08/18 15:47:03 eric Exp $
...
function search_fulltext(&$action) {
  ...
  while(list($kd,$vd) = each($doclist)) {
    ...
    if (strlen($doc->title) > 20)	
      $tdoc[$k]["abrvtitle"] = 
	substr($doc->title,0,12)."... ".substr($doc->title,-5);
    ...
    $tdoc[$k]["locked"] ="";
    if ($doc->isRevisable()) {
      if (($doc->locked > 0) && ($doc->locked == $action->parent->user->id)) 
	$tdoc[$k]["locked"] = $action->GetIcon("clef1.gif",N_("locked"), 20,20);
