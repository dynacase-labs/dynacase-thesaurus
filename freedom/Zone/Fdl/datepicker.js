...
Calendar.prototype.show = function() {
	...
	this.wwrite("<link rel=stylesheet href=\"FDL/Layout/datepicker.css\" type=text/css>");
	this.wwrite("</head>");
	...
	this.wwrite("<TABLE WIDTH='100%' BORDER=0 ><TR><TD ALIGN=center>");
...
Calendar.prototype.cal_header = function() {
	...
	vCode = vCode + "<TR>";
	for (i=0; i<7; i++) {
	  vCode = vCode + "<TH class=\"day\" WIDTH='14%'>"+Calendar.Days[i]+"</TH>";
        }
	vCode = vCode + "</TR>";
...
function show_calendar(event) {
	...
	*/
	...
	// serach where open the window date picker
	...
	with (document.getElementById(p_item))  {
	    if (type == 'text')  xw = xw - (size*7) -200;
	    else  if (type == 'textarea') xw = xw - (cols*7) -200;
	}
	vWinCal = window.open('', 'wchoose', 'resizable=yes,height=220,width=250,left='+xw+',top='+yw);
	vWinCal.opener = self;
	wichoose=vWinCal;
	ggWinCal = vWinCal;
