...
function dateinterval(&$action) {
  ...
  $action->lay->Set("period",
		    sprintf(_("period from %s to %s"),
			    strftime("%d %B %Y",$fdate),
			    strftime("%d %B %Y",$tdate)));

  if ($tdate < $fdate) $action->ExitError(sprintf(_("the end date (%s) is before the begin date (%s)"),
						   strftime("%d %B %Y %T",$tdate),
						  strftime("%d %B %Y %T",$fdate)));						  

  return array("fromdate"=>$fdate,
	       "todate"=>$tdate);
}
?>