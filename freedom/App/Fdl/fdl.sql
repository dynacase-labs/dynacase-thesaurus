...
if ((TG_OP = ''UPDATE'') OR (TG_OP = ''INSERT'')) then
  if  NEW.doctype != ''T'' then
	...
	insert into docread(id,owner,title,revision,initid,fromid,doctype,locked,allocated,icon,lmodify,profid,usefor,revdate,version,cdate,adate,comment,classname,state,wid,attrids,postitid,cvid,name,dprofid,prelid,atags,confidential,ldapdn,values,fulltext,svalues) values (NEW.id,NEW.owner,NEW.title,NEW.revision,NEW.initid,NEW.fromid,NEW.doctype,NEW.locked,NEW.allocated,NEW.icon,NEW.lmodify,NEW.profid,NEW.usefor,NEW.revdate,NEW.version,NEW.cdate,NEW.adate,NEW.comment,NEW.classname,NEW.state,NEW.wid,NEW.attrids,NEW.postitid,NEW.cvid,NEW.name,NEW.dprofid,NEW.prelid,NEW.atags,NEW.confidential,NEW.ldapdn,NEW.values,NEW.fulltext,NEW.svalues);
     end if;
  end if;
